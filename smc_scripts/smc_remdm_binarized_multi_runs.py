...
def main(args):
    ...
    remasking_schedule = args.remasking_schedule
    remasking_kwargs={
        "eta_cap": args.remasking_eta_cap,
        "eta_rescale": args.remasking_eta_rescale,
    }


    # load pretrained model
    pretrained_model = ReMaskingDiffusion(
        ...
        remasking_schedule=remasking_schedule,
        remasking_kwargs=remasking_kwargs,
    ).to(device)
...
parser.add_argument("--model_type", type=str, default="remdm") # This is not to be modified. But for args.json based filtering.
...
parser.add_argument("--remasking_schedule", type=str, default="max_capped", choices=['max_capped', 'rescaled'])
parser.add_argument("--remasking_eta_cap", type=float, default=0.1)
parser.add_argument("--remasking_eta_rescale", type=float, default=0.1)
