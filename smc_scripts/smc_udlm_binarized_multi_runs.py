...
def main(args):
    ...
    pretrained_model = UniformDiffusion(
        ...
        noise_schedule=args.masking_schedule,
        num_timesteps=num_timesteps,
        discretization_schedule=args.discretization_schedule,
    ).to(device)

    # load model weights
    pretrained_model.load_state_dict(torch.load('model_weights/udm_binarized_mnist_256_copy.pth'))
...
parser.add_argument("--model_type", type=str, default="udlm") # This is not to be modified. But for args.json based filtering.
